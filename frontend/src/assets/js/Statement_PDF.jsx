...
export const StatementPDFView = ({
  ...
  return (
    <div
      className="w-[1060px] bg-white p-6 font-sans text-slate-800"
        ...
        <table className="w-full border-collapse text-[11px] text-slate-800">
          <thead>
            <tr className="bg-[#1a365d] text-white font-bold uppercase tracking-wider text-center">
              ...
              <th className="border border-slate-300 px-3 py-3 w-[14%]">
                Date
              </th>
              <th className="border border-slate-300 px-4 py-3 text-left w-[38%]">
                Description
              </th>
              <th className="border border-slate-300 px-3 py-3 text-right w-[14%]">
                Invoiced Amount
              </th>
              <th className="border border-slate-300 px-3 py-3 text-right w-[14%]">
                Payments Received
              </th>
              <th className="border border-slate-300 px-3 py-3 text-right w-[14%] font-black">
                Running Balance
              </th>
            </tr>
          </thead>
          <tbody>
            {rows.length > 0 ? (
              rows.map((row, idx) => (
                <tr
                  key={idx}
                  className="border-b border-slate-300 even:bg-slate-50/50 hover:bg-slate-100/10 transition-colors"
                >
                    ...
                    {row.credit > 0 ? (
                      <div>
                        <div className="font-bold text-slate-800">
                          PAYMENT RECEIVED - {row.client?.toUpperCase()} -{" "}
                          {row.invoiceNumber?.toUpperCase()}
                        </div>
                        {row.description &&
                          row.description !== "CASH RECEIVED" && (
              ...
              <tr>
                <td
                  colSpan={6}
                  className="border border-slate-300 px-4 py-8 text-center font-semibold text-slate-400 italic"
                >
          ...
          <tfoot>
            <tr className="bg-slate-100 font-black text-slate-900 border-t-2 border-slate-400">
              <td
                colSpan={3}
                className="border border-slate-300 px-4 py-3.5 text-center uppercase tracking-wider text-[#1b4fd8] font-bold"
              >
                Totals
              </td>
...
export const generateStatementPDF = async (
  ...
  try {
    ...
    const element =
      container.querySelector("#statement-pdf-content") || container;
    const options = {
      margin: [8, 8, 8, 8],
